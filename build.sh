...
function copy_root_2_system()
{
    ...
    rm -fr $TMP_OUT_DIR/system/app/Provision
    rm -fr $TMP_OUT_DIR/system/priv-app/CyanogenSetupWizard
    ...
    cp -arp $ROOT_DIR/device/hardkernel/odroidxu3/gapps_tv/system/* $TMP_OUT_DIR/system/

    rm -fr $TMP_OUT_DIR/system/priv-app/SetupWraith

    sed -i s/ro.product.board\=odroidxu3/ro.product.board\=SVPDTV15/g $TMP_OUT_DIR/system/build.prop
    sed -i s/ro.product.device\=odroidxu3/ro.product.device\=SVP-DTV15/g $TMP_OUT_DIR/system/build.prop
